#include "utils.h"
pcl::PointCloud<pcl::PointXYZINormal>::Ptr Utils::livox2PCL(const livox_ros_driver::CustomMsg::Ptr msg, int filter_num, double min_range, double max_range)
{
...
ros::Time Utils::getTime(const double &sec)
{
    ros::Time time_msg;
    time_msg.sec = static_cast<int32_t>(sec);
    time_msg.nsec = static_cast<uint32_t>((sec - time_msg.sec) * 1e9);
